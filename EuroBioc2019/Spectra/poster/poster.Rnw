...
\definecolor{hexblue}{HTML}{A9BEE0}

\setbeamercolor*{itemize item}{fg=hexblue}
\setbeamercolor*{itemize subitem}{fg=hexblue}
\setbeamercolor*{block title}{fg=hexblue}
...
# Select all MS2 spectra for a [M+Na]+ ion of
# Fructose.
...
@
  ...
  \begin{columns}
    ...
    \begin{column}{.9\textwidth}
        ...
        Install the \Rpackage{Spectra} package using:

<<r install, eval = FALSE >>=
BiocManager::install(c("RforMassSpectrometry/MsCoreUtils", "RforMassSpectrometry/Spectra"))
@
    \end{column}
    \begin{column}{.05\textwidth}
    \end{column}
