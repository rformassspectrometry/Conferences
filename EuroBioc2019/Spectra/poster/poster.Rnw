...
\author[Gatto et al.]{
    \large Laurent Gatto$^{1}$, Johannes Rainer$^{2}$ and Sebastian Gibb$^{3}$
}

\institute[]{
  \begin{small}
    $^{1}$ de Duve Institute, UCLouvain, Brussels, Belgium \\
    $^{2}$ Institute for Biomedicine, Eurac Research, Italy \\
    $^{3}$ Department of Anaesthesiology and Intensive Care, University Medicine Greifswald, Germany \\
    ~
  \end{small}
}
...
@
  ...
  \begin{columns}
    ...
    \begin{column}{.7\textwidth}
      ...
      \begin{block}{}
        To use the \Rpackage{Spectra} package install
        the \textbf{R for Mass Spectrometry} packages and
